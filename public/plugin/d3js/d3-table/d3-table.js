    ...
    function D3Table(id, option) {
        ...
        obj.__proto__ = {
            ...
            __createRowHead: function () {
                ...
                var head = headColumns.selectAll(GRID_CSS.headCellCss.prefixjoin(' .')).data(this.columns).enter().append('td').attr('class', GRID_CSS.headCellCss.join(' '))
...
//                            }).dispatch('click');
            ...
            __createRowView: function () {
                ...
                this.__gridViewBody = this.__gridViewport.append('div').attr('class', 'grid-view-body').style('overflow-x', 'hidden')
                ...
                switch (type) {
                    case 'angle':
                        me.__setTransitionStyleTween(transition, function (data, i) {
                            ...
                            var transition = d3.select(this).style('opacity', '1').transition().duration(100).on('end', function (d) {
            ...
            __update: function (rowselection) {
                ...
                var transition = rowselection.transition().duration(100).delay(function (d, i) {
                    return me.delayFun(d, i, 10);
                    ...
                    if (me.sortColumn && me.options.cellBack) {
                        d3.select(this).select('.grid-row-back')
                            .transition().duration(100).style('width', function (d) {
...
//                        }
            ...
            removeRow:function (data) {
                ...
                for (var i = 0; i < this.nowData.length; i++) {
                    var prevD = this.nowData[i];
                    if (this.equalsFun(data, prevD)) {
                        this.nowData.splice(i,1);
                    }
                }
                this.__updateRow(this.nowData);
...
//                this.__sort = null;
            ...
            __changeDom: function (data) {
                ...
                var row= this.tbody.selectAll('.'+GRID_CSS.rowCss[0].trim()).data(data);
                ...
                if (this.options.animation) {
                    ...
                    var transition = this.rowselection.transition().duration(100).delay(function (d, i) {
                        return me.delayFun(d, i, 10);
                    }).on('end', function (d) {
                ...
                var removeduration = exitRow.transition().duration(100).remove().on('end', function () {
                });

                me.__setTransitionStyleTween(removeduration, function (data, i) {
                    return function (t) {
                        return "translate3d" + '(' + (-t) * 100 + '%,0px,0) ';
                    }
                });
            ...
            createCell: function (row) {
                ...
                var cellTd = cell.enter().append('td').attr('class', function (d, i) {
                    ...
                    if (d.width) {
                        ...
                        var head = me.__headTable.selectAll('.grid-head-cell[data-index=' + d.dataIndex + ']').node();
...
//            		});
		...
		obj.D3Scroll = true;//obj.D3Scroll?obj.D3Scroll:false;
