...
//		}
			...
			if (arguments.length == 1 && typeof(args) == "object") {
				for (var key in args) {
					...
					if (args[key] != undefined) {
						if(typeof(args[key])!='function'){
							result = result.replace(reg, this.escape(args[key]));
						}
				...
				for (var i = 0; i < arguments.length; i++) {
					...
					if (arguments[i] != undefined) {
						if(typeof(args[key])!='function'){
							result = result.replace(reg, this.escape(arguments[i]));
						}
					} else {
						result = result.replace(reg, '');
					}
	...
	String.prototype.escape = function(v){
		if(!this.$es){
			this.$es = $('<div>');	
		}
		if(this.$es){
			return this.$es.text(v).html();
		}
	}
...
//		}
	...
	var Constants = {
		...
		encodeObjToJSON : function(obj) {
			...
			return this.__encodeURI(json);
		},
		__encodeURI : function(params) {
			return encodeURIComponent(encodeURIComponent(params));
		},
		decodeURIToObj : function(encodeString) {
			return JSON
					.parse(decodeURIComponent(decodeURIComponent(encodeString)));
		},
		empty : function(v) {
			switch (typeof v) {
				case 'undefined' :
			...
			var eachObj = function(obj) {
				...
				for (var p in obj) { // 方法
					if (_.isObject(obj[p])) {
						...
						if (_.isNull(obj[p]) || (typeof obj[p] == 'undefined')
								|| obj[p] === "") {
							delete obj[p];
						}
					}
		 ...
		 * @param {}
		 *            menuId 对应菜单ID
		 * @param {keyword:'xxx',text:'xxx',renderer:function, }
		 *            authority 权限对象
		 * @param {id:xx,name:'xxxx' }
		 *            row 选择参数 只有表格使用
		 * @gridControl renderer的参数e 只有表格使用
		 * @return String
		 */
		setAuthority : function() {
			var menuId, authority, row, callback;
			if (arguments.length > 3) {
				menuId = arguments[0], authority = arguments[1], row = arguments[2], callback = arguments[3];
			}
			if (arguments.length == 3) {
				menuId = arguments[0], authority = arguments[1], callback = arguments[2];
			}
			...
			$.ajax({
				...
				success : function(result) {
					...
					if (!result.data)
						return;
					var authority$ = [];
					var authorityMap = result.data;// Constants.decodeURIToObj(result.data);

					$.each(authority, function(i, d) {

						var id = Math.round(Math.random() * 100000000)
								+ new Date().getTime();
						d.btnId = id + '-' + d.keyword;

						if (authorityMap[d.keyword]) {
							if (d.renderer) {
								var fn = typeof d.renderer == "function"
										? d.renderer
										: window[d.renderer];
								var str = fn.call(d, row);
								if (str) {
									var $str = $(str);
									if (!$str.attr('id')) {
										$str.attr('id', d.btnId);
									}
									$str.attr('data-keyword', d.keyword).attr(
											'name', d.name);
									authority$.push($str);
									$str.on('click.authority', function() {
												if (d.click) {
													d.click($(this), d, row, e);
												}
											})
								} else {
									console.log('renderer 没有返回String');
								}

							} else {
								var str = '<a id="{btnId}" href="javascript:void(0);" name="authority-button" class="authority-button {keyword}" data-keyword="{keyword}" data-name="{name}" data-id="{id}" >{text}</a>'
										.format(d), str = row
										? str.format(row)
										: str;
								var $str = $(str);
								$str.on('click.authority', function(e) {
											if (d.click) {
												d.click($(this), d, row, e);
											}
										})
							...
							// $('body').on('click.authority', "#" + d.btnId,
							// function(e) {
							//
							// });
						}
		...
		domHasClass : function($dom, classArray) {
			var flag = false;
			if (!classArray)
				return;
			for (var i = 0; i < classArray.length; i++) {
				if ($dom.hasClass(classArray[i])) {
					flag = true;
					break;
				}
			}
		...
		setAuthority : function(menuId) {
			...
			if (menuId) {
				return;// 关闭权限管理
				$(".authority-btn").each(function() {
							if (!me.domHasClass($(this), _buttons[menuId])) { // 不包含移除
								$(this).off().remove();
							}
						});

			} else {
				$(".authority-btn").off().remove();
			}
		}
	}
	...
	Constants.setAuthority(Share.data('menuId'));
