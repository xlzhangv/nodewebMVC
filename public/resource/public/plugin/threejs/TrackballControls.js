...
THREE.TrackballControls = function ( object, domElement ) {
    ...
    this.moveTo = function (vector) {
        ...
        // vector.multiplyScalar( _eye.length() * _this.panSpeed );
        var mid = new THREE.Vector2().lerp(vector.clone(), 0.5);
        var splineCurve = new THREE.QuadraticBezierCurve3(new THREE.Vector2(), mid, vector);
        var points = splineCurve.getPoints(5);

        var pan = _eye.clone().cross( _this.object.up ).setLength( -vector.x );
        pan.add( _this.object.up.clone().setLength( vector.y ) );
